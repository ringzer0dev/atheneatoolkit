...
def parse_args():
    ...
    parser.add_argument("-s", "--savefile", help="Save packets into PCAP File", action = 'store_true')
...
def output(err, monchannel):
    ...
    if err:
        ...
        print('['+G+'+'+G+'] '+C+'Athenea AP Scanner running on '+mon_iface+' channel: '+P+monchannel+END)
    if len(clients_APs) > 0:
        print(B+'   |====Clients====|--->|====Gateway====|  |ch|   |ESSID|'+END)
    # Print the clients list
    with lock:
        for ca in clients_APs:
            if len(ca) > 3:
                print(G+'['+B+'*'+B+G+'] '+P+ca[0]+O+' - '+P+ca[1]+O+' - '+ca[2].ljust(2)+' - '+T+ca[3]+END)
            else:
                print(G+'['+B+'*'+B+G+'] '+P+ca[0]+O+' - '+P+ca[1]+O+' - '+ca[2]+END)
    if len(APs) > 0:
        print(C+'\n    |=====BSSID=====|  |ch|  |===ESSID===| '+END)
        print(G+'===============================================')
    with lock:
        for ap in APs:
            print(G+'['+B+'*'+B+G+'] '+O+ap[0]+G+' - '+ap[1].ljust(2)+' - '+T+ap[2]+G)
    print('')
...
def cb(pkt):
    ...
    if pkt.haslayer(Dot11):
        if pkt.addr1 and pkt.addr2:
            ...
            if args.savefile:
                wrpcap("capture.pcap", pkt, append = True, sync = True)
...
def stop(signal, frame):
    if monitor_on:
        raw_input(G+"[*] Press "+O+"[return]"+G+"to exit")
        hop = threading.currentThread()
        hop.do_run = False
...
def main():
    ...
    try:
        ...
        print(B+'\n['+G+'*'+G+B+']Gracefully exiting, thanks for using'+O+' Athenea AP Scanner'+G+' ... '+END)
        stop()
        athenea.APScan.menu()

    print(G+"[*] Sniffing with "+mon_iface)
